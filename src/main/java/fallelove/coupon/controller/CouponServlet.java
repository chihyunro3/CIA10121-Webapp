...
public class CouponServlet extends HttpServlet{
	...
	public void doPost(HttpServletRequest req, HttpServletResponse res)
		...
		if ("getOne_For_Update".equals(action)) {
			...
			CouponVO couponVO = couponSvc.getOneCoupon(coupNo);
			/***************************3.查詢完成,準備轉交(Send the Success view)************/
			String param = "?coupNo=" +couponVO.getCoupNo()+
		...
		/***************************1.接收請求參數 - 輸入格式的錯誤處理**********************/
			...
			BigDecimal bd = new BigDecimal(coupDisc);
//			BigDecimal coupDisc = new BigDecimal(req.getParameter("coupDisc"));
			try {
				if (coupDisc == null || coupDisc.trim().length() == 0) {
				errorMsgs.put("coupCond", "請輸入折扣(若為九折請輸入0.9)");
				}
			...
			Integer coupRelStat = Integer.valueOf(req.getParameter("coupRelStat"));
			...
			/***************************2.開始修改資料*****************************************/
			CouponService couponSvc = new CouponService();
			CouponVO couponVO = couponSvc.updateCoupon(coupNo, coupName, coupCond, bd, coupRelDate, coupExpDate, coupRelStat);
